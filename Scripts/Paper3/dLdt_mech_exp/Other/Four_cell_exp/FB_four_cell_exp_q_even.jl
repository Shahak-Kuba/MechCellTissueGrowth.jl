...
for a_value in resting_lengths
    ...
    for m_value in m_vals
        ...
        Domain = MCTG.DomainProperties_t(N=4, m = m_value)
...
f4 = Figure(size=(1200, 400))
ax1 = Axis(f4[1,1], xlabel="Time", ylabel="Length", title="m = $(m_vals[1])")
ax2 = Axis(f4[1,2], xlabel="Time", ylabel="Length", title="m = $(m_vals[2])")
ax3 = Axis(f4[1,3], xlabel="Time", ylabel="Length", title="m = $(m_vals[3])")
all_axes = [ax1, ax2, ax3]
...
for jj in 1:length(m_vals)
    ...
    lines!(ax, sol.t, cell_lengths[:,4], linewidth=3,color=line_colors[4], linestyle=:solid, label = "Cell 4")
    if jj == 3
...
save("FB_four_cell_Lt_even.png", f)
save("FB_four_cell_dLdt_longer_even.png", f2)
save("FB_four_cell_final_cell_length_even.png", f3)
save("FB_four_cell_all_cell_lengths_even.png", f4)