C₁ = (L, λ₁, λ₂, x₁, x₂) -> ( (2 .+ λ₂) .*(x₁ .- (L./3)) .- (x₂ .- ((2 .* L) ./3))) ./ (λ₂ .- λ₁)
C₂ = (L, λ₁, λ₂, x₁, x₂) -> ( (x₂ .- ((2 .* L) ./3)) .- (2 .+ λ₁) .*(x₁ .- (L./3)) ) ./ (λ₂ .- λ₁)
...
# Exact solutions
L₁ = (k, η, t, L, λ₁, λ₂, x₁, x₂) -> L./3 .+ C₁(L, λ₁, λ₂, x₁, x₂) .* exp.((k.*λ₁.*t)./η) .+ C₂(L, λ₁, λ₂, x₁, x₂) .* exp.((k.*λ₂.*t)./η);
L₂ = (k, η, t, L, λ₁, λ₂, x₁, x₂) -> (2 .* L)./3 .+ (2 .+ λ₁).*C₁(L, λ₁, λ₂, x₁, x₂).*exp.((k.*λ₁.*t)./η) .+ (2 .+ λ₂).*C₂(L, λ₁, λ₂, x₁, x₂) .* exp.((k.*λ₂.*t)./η);
...
Domain = MCTG.DomainProperties_t(N=3, m = 2, domain_type = "1D")
...
SimTime = MCTG.SimTime_t(Tmax=20, δt=0.001, event_δt=0.001)

IC = [x₁/2, x₁, x₁ + (x₂-x₁)/2, x₂, x₂ + (L - x₂)/2,  L]
#l0 = x₂
#IC = reverse(collect(LinRange(0, L, Domain.N*Domain.m + 1)))
#pop!(IC)
#reverse!(IC)
NumSaveTimePoints = 1001
sol = MCTG.FixedBoundarySimulation_given_IC(IC, Domain, CellMech, SimTime, Prolif, Death, Embed, ProlifEmbed, 1, NumSaveTimePoints);
# discrete model cell length
CB_1 = [sol.u[ii][1,Domain.m + 1] for ii in eachindex(sol.t)]
CB_2 = [sol.u[ii][1,(Domain.N-1) * Domain.m + 1] for ii in eachindex(sol.t)]


t = LinRange(0, SimTime.Tmax, 1001);

F = Figure(size=(600,600))
ax = Axis(F[1,1], aspect=1, xlabel=L"t", ylabel=L"x\;(t)", limits=(-1,21, 1, 11))
lines!(ax, t, L₁(k, η, t, L, λ₁, λ₂, x₁, x₂), linewidth=3, label=L"\x_{1}(t)", color=:navy)
lines!(ax, t, L₂(k, η, t, L, λ₁, λ₂, x₁, x₂), linewidth=3, label=L"\x_{2}(t)", color=:darkorange)
