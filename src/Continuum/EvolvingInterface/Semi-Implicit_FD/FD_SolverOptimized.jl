...
function FD_SolveContinuumLim_Polar_Optimized(T, D, kf, A, ρ₀, Tmax, growth_dir, myR)
...
"""
x = FD_InitialBoundary("square", 100, θ, 240) 
D = 30;
kf = 20.0;
A = 0.00;
ρ₀ = 0.05;
...
CairoMakie.lines!(ax,R[:,1], linewidth=2)

for index in [1, 3001, 5001, 8001, 10001]
    x = R[index,:] .* cos.(θ)
    y = R[index,:] .* sin.(θ)  
    CairoMakie.lines!(ax, x, y, color=ρ[index,:],linewidth=3,colormap=:jet, colorrange=[0.05,0.75])
end

display(f)

x = 100ones(100)
x2 = Vector(LinRange(0.0,2π,101))
pop!(x2)
r = 100 .* (sin.(x2) .+ 1) 
