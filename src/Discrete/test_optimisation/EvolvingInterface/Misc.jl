...
# Helper function to convert SimResults_t fields to regular arrays
function convert_to_regular_matrix_vector(sim_results::SimResults_t)
    u_x_positions    = [el[1, :] for el in sim_results.u]
    u_y_positions    = [el[2, :] for el in sim_results.u]
    ∑F_vectors       = sim_results.∑F
    Density_matrices = sim_results.Density
    ψ_vectors        = sim_results.σ
    Κ_matrices       = sim_results.Κ
